package com.mygdx.game.supmario.actors;
...
public class Mario extends RigidBody {
    ...
    private void handleInput() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.X) && isFireMario && fireTimer > fireInterval && currentState != State.CROUCHING) {
            ...
            float y = 0.8f;
            GameManager.instance.getAssetManager().get("mario/audio/sfx/fireball.ogg", Sound.class).play();
        ...
        if (smallJump && jumpSoundTimer > 0.15f) {
            if (bigJump) {
                assetManager.get("mario/audio/sfx/jump_super.wav", Sound.class).play();
            }
            else {
                assetManager.get("mario/audio/sfx/jump_small.wav", Sound.class).play();
            }
            smallJump = false;
            bigJump = false;
        }
    ...
    public void update(float delta) {
        ...
        if (die) {
            if (!isDead) {
                assetManager.get("mario/audio/sfx/mariodie.wav", Sound.class).play();
        ...
        if (other.getFilter().categoryBits == GameManager.ENEMY_WEAKNESS_BIT) {
            ...
            if (isInvincible) {
                ...
                if (!isGrownUp) {
                    ...
                    assetManager.get("mario/audio/sfx/powerdown.wav", Sound.class).play();
                    shrink = true;
                }
            }
        }
            ...
            if (item.getName().equals("mushroom")) {
                if (!isGrownUp) {
                    assetManager.get("mario/audio/sfx/powerup.wav", Sound.class).play();
                    growUp = true;
                }
                else {
                    assetManager.get("mario/audio/sfx/stomp.wav", Sound.class).play();
                }
